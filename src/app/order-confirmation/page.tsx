...
export default function OrderConfirmationPage() {
  ...
  const itemsTextList = cart.map((item: any) => {
    if (item.isByWeight) {
      return `• ${item.name} - ${item.weight} גרם`;
    }
    if (item.averageWeightPerUnit && item.estimatedUnitPrice) {
      const estimatedPrice = (item.estimatedUnitPrice * item.quantity).toFixed(2);
      return `• ${item.name} - ${item.quantity} יחידות (מחיר משוער: ₪${estimatedPrice})`;
    }
    return `• ${item.name} - ${item.quantity} יחידות`;
  });

  return (
    <div className="container mx-auto p-6 max-w-2xl bg-white shadow-lg rounded-lg my-10 text-right">
      ...
      <div className="border-t border-b border-gray-200 py-4 mb-6">
        ...
        <p className="text-gray-600">שלחנו עותק של ההזמנה לטלגרם, ניצור איתך קשר בהקדם במידת הצורך.</p>
      </div>

      <div className="mb-6">
        <h3 className="text-xl font-semibold mb-3">סיכום הזמנה</h3>
        <ul className="list-none space-y-2">
          {itemsTextList.map((text, index) => (
      ...
      <div className="text-center">
        <Link href="/order" className="btn-primary text-lg px-8 py-3">
          חזרה לחנות
        </Link>
      </div>
    </div>
